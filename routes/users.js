...
router.post('/:userid/search/:id/purchase', function (req, res, next) {
  console.log(req.params);
  Book.findById({ _id: req.body.id}).then(function (book) {
    // delete book.image;
...
router.put('/books/request', function (req, res, next) {
  // console.log(req.params.id);
  Book.find({ name: req.body.name }).then(function (book) {
    if (book.length != 0) {
      res.send({ type: 'Book already exists', });
    } else {
